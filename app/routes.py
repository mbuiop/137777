...
def profile():
    ...
    return render_template('profile.html', messages=messages, uploads=uploads)
...
def dashboard():
    ...
    ai_stats = ai.get_stats() if hasattr(ai, 'get_stats') else {}
...
def upload_file():
    ...
    if file_ext not in current_app.config.get('ALLOWED_EXTENSIONS', {'txt', 'pdf', 'docx', 'csv'}):
    ...
    flash(f'✅ {extracted} مورد یادگیری استخراج شد', 'success')
